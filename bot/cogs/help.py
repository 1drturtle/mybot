...
class CustomHelp(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        title = f'{self.cog.bot.name} Help'
        ...
        await command_menu.start(self.context, channel=self.get_destination())

    async def send_cog_help(self, cog):
        embed = create_default_embed(self.context)
        title = f'{self.cog.bot.name} Help - `{cog.qualified_name}`'.strip()
        footer = f'An underlined command has sub-commands.\n' \
                 f'See {self.clean_prefix}help <command name> for more details on individual commands.'
    ...
    async def send_group_help(self, group):
        title = f'{self.cog.bot.name} Help - `{self.get_command_signature(group)}`'.strip()
        footer = f'An underlined command has sub-commands.\n' \
                 f'See {self.clean_prefix}help <command name> for more details on individual commands.'
    ...
    async def send_command_help(self, command):
        embed = create_default_embed(self.context)
        embed.title = f'{self.cog.bot.name} Help - `{self.get_command_signature(command).strip()}`'
